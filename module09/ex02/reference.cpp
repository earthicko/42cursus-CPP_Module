...
auto operator==(const group_iterator<Iterator1> &lhs,
				const group_iterator<Iterator2> &rhs) -> bool
{
...
template <typename Iterator>
auto operator-(const group_iterator<Iterator> &lhs,
...
auto merge_insertion_sort_impl(RandomAccessIterator first,
							   ...
							   Compare compare)
{
	...
	merge_insertion_sort(
		make_group_iterator(first, 2), make_group_iterator(end, 2), compare);
	...
	for (int k = 0;; ++k)
	{
		...
		while (true)
		{
			auto insertion_point = std::upper_bound(
				chain.begin(), it->next, it->it, [=](auto lhs, auto rhs) {
...
auto merge_insertion_sort(RandomAccessIterator first,
						  RandomAccessIterator last,
						  Compare compare = {}) -> void
{
	merge_insertion_sort_impl(
		make_group_iterator(first, 1), make_group_iterator(last, 1), compare);
}
...
int main(int argc, char **argv)
{
	...
	merge_insertion_sort(elements.begin(), elements.end());
